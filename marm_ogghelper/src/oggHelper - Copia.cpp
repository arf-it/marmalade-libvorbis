...
aInternalBuffer(size), iBufferSize(size), rIdx(0), wIdx(0),
	bBufferIsEmpty(true), bBufferIsFull(false), iUnread(0) {}
...
//bool CCircularBuffer::bBufferIsNotEmpty()
//{
//	return iUnread > 0;
//}
//
//bool CCircularBuffer::bBufferIsNotFull()
//{
//	 return iUnread < iBufferSize;
//}
...
int COggVorbisFileHelper::decode()
{
	...
	if(nStatus == OH_PLAYING) current_time = ov_time_tell_func(&vf);
...
bool COggVorbisFileHelper::set_outputStereoMode( STEREO_MODE val )
{
	...
	if (val != STEREO_MODE_MONO)
	{
		...
		if((res == S3E_RESULT_ERROR)||(!bStereo))
		{
